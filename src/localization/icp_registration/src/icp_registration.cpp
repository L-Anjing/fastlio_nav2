...
void IcpNode::pointcloudCallback(
  ...
  if (has_intensity) {
    ...
    cloud_in_->width = static_cast<uint32_t>(cloud_in_->size());
    cloud_in_->height = 1;
  ...
  if (cloud_in_->empty()) {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                         "Received empty point cloud for ICP input.");
    return;
  }
...
void IcpNode::initialPoseCallback(
  ...
  bool has_laser_to_odom_tf = false;
  try {
    ...
    has_laser_to_odom_tf = true;
  } catch (tf2::TransformException &ex) {
    RCLCPP_WARN(this->get_logger(),
                "Missing TF for ICP map->odom init (target=%s, source=%s), "
                "fallback to identity laser->odom for bootstrap: %s",
                laser_frame_id_.c_str(), range_odom_frame_id_.c_str(),
                ex.what());
  }
  if (!has_laser_to_odom_tf) {
    laser_to_odom.setIdentity();
  }
