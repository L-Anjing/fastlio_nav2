...
def generate_launch_description():
    ...
    fastlio_mid360_params = os.path.join(nav_bringup_dir, 'config', 'reality', 'fastlio_mid360_real.yaml')
    fastlio_rviz_cfg_dir = os.path.join(nav_bringup_dir, 'rviz', 'fastlio.rviz')
    ##################################### FAST_LIO parameters end #####################################

    ################################### POINT_LIO parameters start ####################################
    pointlio_mid360_params = os.path.join(nav_bringup_dir, 'config', 'reality', 'pointlio_mid360_real.yaml')
    pointlio_rviz_cfg_dir = os.path.join(nav_bringup_dir, 'rviz', 'pointlio.rviz')
    ...
    pointcloud_downsampling_config_dir = os.path.join(nav_bringup_dir, 'config', 'reality', 'pointcloud_downsampling_real.yaml')
    ############################# pointcloud_downsampling parameters start ############################
    ...
    bringup_LIO_group = GroupAction([
        ...
        GroupAction(
            ...
            actions=[
            # Bridge FAST_LIO tf frames (camera_init/body) to nav stack frames (odom/base_link).
            Node(
                package='tf2_ros',
                executable='static_transform_publisher',
